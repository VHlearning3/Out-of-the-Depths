...
public static class ItemModelTools
{
    ...
    // Item asset name -> model file name (without extension) under Art/Models. Add a line per new model.
    private static readonly (string item, string model)[] KnownModels =
    {
        ("Item_FirstRoomKey", "gold_key"),
        ("Item_SymbolKey", "rune_key"),
    };
    ...
    {
        foreach (var (itemName, _) in KnownModels)
        {
    ...
    {
        int assigned = 0;
        foreach (var (itemName, modelName) in KnownModels)
        {
            ItemDefinition item = ItemTools.Load(itemName);
    ...
    private static bool BakeModel(GameObject root, bool destroyPlaceholder = true)
    {
        ...
        if (visual != null && bakedFor.objectReferenceValue == item)
        {
            PickupItem.FitItemModel(root.transform, item, visual.gameObject, null, so.FindProperty("modelSize").floatValue);
            return true;
        }

        var model = (GameObject)PrefabUtility.InstantiatePrefab(item.WorldModel, root.scene);
        ...
        // Record what the visual is now. Play mode leaves it alone while Item matches, and swaps it if an instance
        // overrides Item to something else.
