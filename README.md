...
- Sounds (`Door → Sounds`, real CC0 metal-hatch recordings pre-wired, swap any): `Unlatch Sound` (a dungeon bolt) as it starts moving, `Groan Sound` (a long metal groan that rings on, random pitch) with it, `Move Loop` grinding while it moves (volume and pitch follow the speed), `Open Stop Sound` when it reaches open, `Close Stop Sound` (a slam with a long reverb tail) when it lands shut, again quieter on each bounce, `Locked Sound` when you try it locked. `Pitch` (0.85) makes everything deeper and heavier and `Reverb` (Hangar) lets it roll down the corridor. `Muffle Cutoff` low-passes everything so it sounds underwater; `Hearing Range` is how far it carries. Events: `On Opened`, `On Closed`.
...
| Pick up | `Pickup_Placeholder → Pickup Item → Pickup Sound` (the SFX pickup clip is pre-wired on the prefab) |
...
- **Player/** — `SwimController` (movement + camera feel), `SwimAudio` (water-rush loop + strokes that follow your speed), `PlayerPanic` (panic 0..1 from chases, bites and low health: breathing, heartbeat, heavier sway, tremble, adrenaline; the HUD vignette beats in time with it), `HungerSystem`, `HealthSystem`, `DamageManager`, `DeathManager` (respawn), `PlayerInteractor` (E prompt), `PlayerInventory` (5-slot hotbar, 1-5 / wheel), `PlayerBody` (the "is this collider the player?" check for triggers), `PlayerTrail` (breadcrumbs the chase pack follows)
