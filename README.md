...
The GDD's tie-the-pieces-together step is a minigame (`Bone Key Tying`, on the `Knot` child the builders put under the seaweed): once the three fragments are in the seaweed the board opens with the pieces in a row and a strand of seaweed on the cursor; hold the mouse button and wind round a joint steadily and inside its ring: every full turn is a wrap, winding back unwinds, too fast and the strand slips, too slow or outside the ring and it comes loose. With `Wraps Per Joint` wraps (4) on, pull it tight: drag away from the joint (a bobbing arrow shows the way), hold the tension in the green band for `Hold Seconds` until the meter fills, then let go; the green sits somewhere new for every joint (early or too quick = a wrap comes loose, too hard = the strand snaps). Both joints tied, the finish plays (the knots creak tighter and the board trembles, the pieces slam together in a white flash, two bursts of sparks, the key rising in a halo with its name), then the bone key is spawned as a pickup and shown in the inspect view like any pickup (E takes it; `Key Pickup` on the component is the prefab, empty = straight into the inventory), and `On Tied` fires (the seaweed swoops). Escape puts it down with the wraps kept; press E on the seaweed to carry on. Each piece has its own picture (`Piece Icons`, rendered from the three piece models by the builders; empty = the fragment icon), the key is its icon; sounds (`Sound/Puzzles`, CC0), colours, sizes and texts are on the component.
